...
public class BoardInsertController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//enctype이 multpart/form-data로 잘 전송되었을 경우 전반적인 내용이 실행되도록 코드변경이 필요
		if (ServletFileUpload.isMultipartContent(request)) {
			 ...
			 * 단, 업로드시 파일명을 수정해주는게 일반적이다. 그래서 파일명을 수정시켜주는 객체를 제시해야함
			 * => 같은 파일명이 존재할경우 덮어씌워질 수 있고, 파일명에 한글/특수문자/띄어쓰기가 포함될 경우 서버에 따라 문제발생
			 * 
			 * 기본적으로 파일명이 안겹치도록 수정작업해주는 객체 있다.
			 * => DefaultFileRenamePolicy 객체(cos에서 제공)
			 * => 내부적으로 해당 클레스에서 rename()메소드가 실행되면서 파일명 수정된 후 업로드
			 *  rename(원본파일){
			 *  	기존에 동일한 파일명이 존재할경우
			 *  	파일명뒤에 카운팅된 숫자를 붙여줌
			...
			//MultipartRequest multiRequest = new MultipartRequest(request, 저장시킬폴더의 경로, 용량제한, 인코딩값, new DefaultFileRenamePolicy())
