	...
	<!-- 
		*JSP를 이루는 구성인자
		1.스크립팅 원소: JSP페이지에서 자바코드를 직접 기술할 수 있게하는 기능
					ex) 스크립틀릿, 표현식(출력식)등등...
		2. 지시어 : JSP페이지정보에 대한 내용을 표현한다거나 또다른 페이지를 포함할 때 사용
				  JSP 기능을 확장시키는 라이브러리 등록할때도 사용한다.
				  ex) page지시어, include지시어, taglib지시어
		3. 액션태그(표준액션태그, 커스텀액션태그) : xml기술을 이용해서 기존의 jsp문법을 확장하는 기술을 제공하는 태그
			
			>>표준액션태그   : JSP페이지에서 바로 사용가능(별도의 연동이 필요하지 않다)
						   모든 태그명 앞에 jsp: 접두어를 붙임
						   
			>>커스텀액션태그  : JSP페이지에서 바로 사용 불가능(별도의 라이브러리 연동 필요)
						   모든 태그명 앞에 jsp:외의 다른 접두어를 붙임(종류는 다양)
						   가장 대표적으로 유용한 라이브러리 == JSTL
	 -->
